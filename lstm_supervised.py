...
class LSTMModel:
    def __init__(self, seq_len, input_dim, num_class, hidden_size=256,
        ...
        _, rnn_output = tf.nn.dynamic_rnn(rnn_cell, self.input_x, dtype=tf.float32)
        self.logit = tf.layers.dense(rnn_output, units=num_class, activation=None)
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        model = LSTMModel(28, 28, 10)
        sess.run(tf.global_variables_initializer())

        for iter_id in range(10000):
            ...
            if iter_id % 10 == 0:
                print('\r', iter_id, np.mean(acc_ins), end='', flush=True)